...
abstract class ActiveRecordTest extends DatabaseTestCase
{
    ...
    public function testCustomColumns()
    {
        ...
        if ($this->driverName === 'oci') {
            $customer = Customer::find()->select(array('{{customer}}.*', '([[status]]*2) AS [[status2]]'))
    ...
    public function testFindLazyViaTable()
    {
        ...
        $this->assertEquals(1, $order->itemsarray(0)->id);
        $this->assertEquals(2, $order->itemsarray(1)->id);
    ...
    public function testFindEagerViaTable()
    {
        ...
        $this->assertEquals(1, $order->id);
        $this->assertCount(2, $order->books);
        $this->assertEquals(1, $order->booksarray(0)->id);
        $this->assertEquals(2, $order->booksarray(1)->id);
        ...
        $this->assertEquals(2, $order->booksarray(0)->id);
    ...
    public function testDeeplyNestedTableRelation()
    {
        ...
        $this->assertInstanceOf(Item::className(), $itemsarray(0));
        $this->assertInstanceOf(Item::className(), $itemsarray(1));
        $this->assertEquals(1, $itemsarray(0)->id);
        $this->assertEquals(2, $itemsarray(1)->id);
    }
    ...
    public function testDeeplyNestedTableRelation2()
    {
        ...
        $this->assertInstanceOf(Order::className(), $ordersarray(0));
        $this->assertInstanceOf(Order::className(), $ordersarray(1));
        $ids = array($ordersarray(0)->id, $ordersarray(1)->id);
        sort($ids);
        ...
        $this->assertInstanceOf(Order::className(), $ordersarray(0));
        $this->assertEquals(2, $ordersarray(0)->id);
    ...
    public function testJoinWith()
    {
        ...
        $this->assertCount(3, $orders);
        $this->assertEquals(2, $ordersarray(0)->id);
        $this->assertEquals(3, $ordersarray(1)->id);
        $this->assertEquals(1, $ordersarray(2)->id);
        $this->assertTrue($ordersarray(0)->isRelationPopulated('customer'));
        $this->assertTrue($ordersarray(1)->isRelationPopulated('customer'));
        $this->assertTrue($ordersarray(2)->isRelationPopulated('customer'));
        ...
        $this->assertEquals(2, $ordersarray(0)->id);
        $this->assertEquals(3, $ordersarray(1)->id);
        $this->assertFalse($ordersarray(0)->isRelationPopulated('customer'));
        $this->assertFalse($ordersarray(1)->isRelationPopulated('customer'));
        ...
        $this->assertCount(2, $orders);
        $this->assertEquals(1, $ordersarray(0)->id);
        $this->assertEquals(3, $ordersarray(1)->id);
        $this->assertTrue($ordersarray(0)->isRelationPopulated('books'));
        $this->assertTrue($ordersarray(1)->isRelationPopulated('books'));
        $this->assertCount(2, $ordersarray(0)->books);
        $this->assertCount(1, $ordersarray(1)->books);
        ...
        $this->assertCount(1, $orders);
        $this->assertTrue($ordersarray(0)->isRelationPopulated('items'));
        $this->assertEquals(2, $ordersarray(0)->id);
        $this->assertCount(3, $ordersarray(0)->items);
        $this->assertTrue($ordersarray(0)->itemsarray(0)->isRelationPopulated('category'));
        $this->assertEquals(2, $ordersarray(0)->itemsarray(0)->category->id);
        ...
        $this->assertCount(3, $orders);
        $this->assertEquals(1, $ordersarray(0)->id);
        $this->assertEquals(2, $ordersarray(1)->id);
        $this->assertEquals(3, $ordersarray(2)->id);
        $this->assertTrue($ordersarray(0)->isRelationPopulated('books2'));
        $this->assertTrue($ordersarray(1)->isRelationPopulated('books2'));
        $this->assertTrue($ordersarray(2)->isRelationPopulated('books2'));
        $this->assertCount(2, $ordersarray(0)->books2);
        $this->assertCount(0, $ordersarray(1)->books2);
        $this->assertCount(1, $ordersarray(2)->books2);
...
//            ['applyAlias'], // $query->applyAlias('customer', 'id') // _aliases are currently not being populated
    ...
    public function testFindSimpleRelationWithJoin()
    {
        ...
        $this->assertInstanceOf('yiiunit\data\ar\Customer', $customerWithJoinIndexOrderedarray('user1'));
    }
    ...
    public function testInverseOf()
    {
        ...
        $this->assertSame($customer->orders2array(0)->customer2, $customer);
        $customers = Customer::find()->with('orders2')->where(array('id' => array(1, 3)))->all();
        $this->assertSame($customersarray(0)->orders2array(0)->customer2, $customersarray(0));
        $this->assertEmpty($customersarray(1)->orders2);
        // lazy loading
        $customer = Customer::findOne(2);
        $orders = $customer->orders2;
        $this->assertCount(2, $orders);
        $this->assertSame($customer->orders2array(0)->customer2, $customer);
        $this->assertSame($customer->orders2array(1)->customer2, $customer);
        // ad-hoc lazy loading
        $customer = Customer::findOne(2);
        $orders = $customer->getOrders2()->all();
        $this->assertCount(2, $orders);
        $this->assertTrue($ordersarray(0)->isRelationPopulated('customer2'), 'inverse relation did not populate the relation');
        $this->assertTrue($ordersarray(1)->isRelationPopulated('customer2'), 'inverse relation did not populate the relation');
        $this->assertSame($ordersarray(0)->customer2, $customer);
        $this->assertSame($ordersarray(1)->customer2, $customer);

        // the other way around
        $customer = Customer::find()->with('orders2')->where(array('id' => 1))->asArray()->one();
        $this->assertSame($customer['orders2'][0]['customer2']['id'], $customerarray('id'));
        $customers = Customer::find()->with('orders2')->where(array('id' => array(1, 3)))->asArray()->all();
        $this->assertSame($customer['orders2']array(0)array('customer2')array('id'), $customersarray(0)array('id'));
        $this->assertEmpty($customersarray(1)array('orders2'));

        $orders = Order::find()->with('customer2')->where(array('id' => 1))->all();
        $this->assertSame($ordersarray(0)->customer2->orders2, array($ordersarray(0)));
        ...
        $this->assertSame($ordersarray(0)array('customer2')array('orders2')array(0)array('id'), $ordersarray(0)array('id'));
        $order = Order::find()->with('customer2')->where(array('id' => 1))->asArray()->one();
        $this->assertSame($orderarray('customer2')array('orders2')array(0)array('id'), $ordersarray(0)array('id'));

        $orders = Order::find()->with('customer2')->where(array('id' => array(1, 3)))->all();
        $this->assertSame($ordersarray(0)->customer2->orders2, array($ordersarray(0)));
        $this->assertSame($ordersarray(1)->customer2->orders2, array($ordersarray(1)));

        $orders = Order::find()->with('customer2')->where(array('id' => array(2, 3)))->orderBy('id')->all();
        $this->assertSame($ordersarray(0)->customer2->orders2, $orders);
        $this->assertSame($ordersarray(1)->customer2->orders2, $orders);

        $orders = Order::find()->with('customer2')->where(array('id' => array(2, 3)))->orderBy('id')->asArray()->all();
        $this->assertSame($ordersarray(0)array('customer2')array('orders2')array(0)array('id'), $ordersarray(0)array('id'));
        $this->assertSame($ordersarray(0)array('customer2')array('orders2')array(1)array('id'), $ordersarray(1)array('id'));
        $this->assertSame($ordersarray(1)array('customer2')array('orders2')array(0)array('id'), $ordersarray(0)array('id'));
        $this->assertSame($ordersarray(1)array('customer2')array('orders2')array(1)array('id'), $ordersarray(1)array('id'));
    }
    ...
    public function testInverseOfDynamic()
    {
        ...
        $this->assertSame($customer, $orders2array(0)->customer2);
        ...
        $this->assertEquals($customerarray('id'), $orders2array(0)array('customer2')array('id'));

        $orders2 = $customer->getOrders2()->asArray()->one();
        $this->assertEquals($customerarray('id'), $orders2array('customer2')array('id'));
    }
...
//        $this->assertSame(false, $model->bool_col2);
    ...
    public function testIssues()
    {
        ...
        $this->assertCount(3, $orders);
        $this->assertCount(2, $ordersarray(0)->orderItems);
        $this->assertCount(3, $ordersarray(1)->orderItems);
        $this->assertCount(1, $ordersarray(2)->orderItems);
    ...
    public function testLinkWhenRelationIsIndexed2()
    {
        ...
        $this->assertTrue(isset($order->orderItems2array('3')));
    }
    ...
    public function testLinkWhenRelationIsIndexed3()
    {
        ...
        $this->assertTrue(isset($order->orderItems3array('1_3')));
    }
