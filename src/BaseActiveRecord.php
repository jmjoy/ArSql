    ...
    public function unlinkAll($name, $delete = false)
    {
        ...
        if ($relation->via !== null) {
            ...
            if (!empty($viaRelation->where)) {
                $condition = array('and', $condition, $viaRelation->where);
            }
            ...
            if (!$delete && count($relation->link) === 1 && is_array($this->{$b = reset($relation->link)})) {
                ...
                if (!empty($relation->where)) {
                    $condition = array('and', $condition, $relation->where);
                }
                if (!empty($relation->on)) {
                    $condition = array('and', $condition, $relation->on);
                }
                if ($delete) {
    ...
    private function bindModels($link, $foreignModel, $primaryModel)
    {
        foreach ($link as $fk => $pk) {
            ...
            if (is_array($foreignModel->$fk)) { // relation via array valued attribute
                $foreignModel->$fk = array_merge($foreignModel->$fk, array($value));
            } else {
