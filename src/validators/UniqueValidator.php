...
class UniqueValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if (is_array($this->targetAttribute) && count($this->targetAttribute) > 1) {
            ...
            if ($this->comboNotUnique === null) {
                $this->message = ArSql::t('yii', 'The combination {values} of {attributes} has already been taken.');
            } else {
                $this->message = $this->comboNotUnique;
            }
        } else {
            $this->message = ArSql::t('yii', '{attribute} "{value}" has already been taken.');
        }
    }
    ...
    private function modelExists($targetClass, $conditions, $model)
    {
        ...
        if (!$model instanceof ActiveRecordInterface || $model->getIsNewRecord() || $model->className() !== $targetClass::className()) {
            ...
            if ($query instanceof \yii\db\ActiveQuery) {
                ...
                $primaryAlias = array_keys($query->getTablesUsedInFrom())array(0);
    ...
    private function prepareConditions($targetAttribute, $model, $attribute)
    {
        if (is_array($targetAttribute)) {
            ...
            $conditions = array($targetAttribute => $model->$attribute);
        }
