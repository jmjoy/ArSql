...
class DateValidator extends Validator
{
     ...
     *
     * If this property is not set, the default value will be obtained from `ArSql::$app->formatter->dateFormat`, see [[\yii\i18n\Formatter::dateFormat]] for details.
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = ArSql::t('yii', 'The format of {attribute} is invalid.');
        }
        if ($this->format === null) {
            if ($this->type === self::TYPE_DATE) {
                $this->format = ArSql::$app->formatter->dateFormat;
            } elseif ($this->type === self::TYPE_DATETIME) {
                $this->format = ArSql::$app->formatter->datetimeFormat;
            } elseif ($this->type === self::TYPE_TIME) {
                $this->format = ArSql::$app->formatter->timeFormat;
            } else {
                throw new InvalidConfigException('Unknown validation type set for DateValidator::$type: ' . $this->type);
            }
        }
        if ($this->locale === null) {
            $this->locale = ArSql::$app->language;
        }
        if ($this->timeZone === null) {
            $this->timeZone = ArSql::$app->timeZone;
        }
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = ArSql::t('yii', '{attribute} must be no less than {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = ArSql::t('yii', '{attribute} must be no greater than {max}.');
        }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        if ($timestamp === false) {
            ...
            $this->addError($model, $attribute, $this->message, array());
        } elseif ($this->min !== null && $timestamp < $this->min) {
            $this->addError($model, $attribute, $this->tooSmall, array('min' => $this->minString));
        } elseif ($this->max !== null && $timestamp > $this->max) {
            $this->addError($model, $attribute, $this->tooBig, array('max' => $this->maxString));
    ...
    protected function validateValue($value)
    {
        ...
        if ($timestamp === false) {
            ...
            return array($this->tooSmall, array('min' => $this->minString));
        } elseif ($this->max !== null && $timestamp > $this->max) {
            return array($this->tooBig, array('max' => $this->maxString));
        } else {
            return null;
        }
    }
    ...
    private function parseDateValueIntl($value, $format)
    {
        if (isset($this->_dateFormats[$format])) {
            if ($this->type === self::TYPE_DATE) {
                $formatter = new IntlDateFormatter($this->locale, $this->_dateFormatsarray($format), IntlDateFormatter::NONE, 'UTC');
            } elseif ($this->type === self::TYPE_DATETIME) {
                $formatter = new IntlDateFormatter($this->locale, $this->_dateFormatsarray($format), $this->_dateFormatsarray($format), $this->timeZone);
            } elseif ($this->type === self::TYPE_TIME) {
                $formatter = new IntlDateFormatter($this->locale, IntlDateFormatter::NONE, $this->_dateFormatsarray($format), $this->timeZone);
            } else {
                throw new InvalidConfigException('Unknown validation type set for DateValidator::$type: ' . $this->type);
            }
        } else {
        ...
        if ($parsedDate === false || $parsePos !== mb_strlen($value, ArSql::$app ? ArSql::$app->charset : 'UTF-8')) {
            return false;
        }
