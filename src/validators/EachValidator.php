...
class EachValidator extends Validator
{
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        foreach ($value as $k => $v) {
            ...
            if ($model->hasErrors($attribute)) {
                if ($this->allowMessageFromRule) {
                    ...
                    $model->clearErrors($attribute);
                    $this->addError($model, $attribute, $this->message, array('value' => $v));
        ...
        $model->clearErrors($attribute);
        $model->addErrors(array($attribute => $detectedErrors));
    }
    ...
    protected function validateValue($value)
    {
        ...
        foreach ($value as $v) {
            ...
            if ($result !== null) {
                if ($this->allowMessageFromRule) {
                    $result[1]array('value') = $v;
                    return $result;
                } else {
                    return array($this->message, array('value' => $v));
                }
            }
        }
