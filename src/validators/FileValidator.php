...
class FileValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = ArSql::t('yii', 'File upload failed.');
        }
        if ($this->uploadRequired === null) {
            $this->uploadRequired = ArSql::t('yii', 'Please upload a file.');
        }
        if ($this->tooMany === null) {
            $this->tooMany = ArSql::t('yii', 'You can upload at most {limit, number} {limit, plural, one{file} other{files}}.');
        }
        if ($this->wrongExtension === null) {
            $this->wrongExtension = ArSql::t('yii', 'Only files with these extensions are allowed: {extensions}.');
        }
        if ($this->tooBig === null) {
            $this->tooBig = ArSql::t('yii', 'The file "{file}" is too big. Its size cannot exceed {formattedLimit}.');
        }
        if ($this->tooSmall === null) {
            $this->tooSmall = ArSql::t('yii', 'The file "{file}" is too small. Its size cannot be smaller than {formattedLimit}.');
        }
        ...
        if ($this->wrongMimeType === null) {
            $this->wrongMimeType = ArSql::t('yii', 'Only files with these MIME types are allowed: {mimeTypes}.');
        }
    ...
    public function validateAttribute($model, $attribute)
    {
        if ($this->maxFiles != 1) {
            ...
            if ($this->maxFiles && count($files) > $this->maxFiles) {
                $this->addError($model, $attribute, $this->tooMany, array('limit' => $this->maxFiles));
            } else {
    ...
    protected function validateValue($value)
    {
        if (!$value instanceof UploadedFile || $value->error == UPLOAD_ERR_NO_FILE) {
            return array($this->uploadRequired, array());
        }
                ...
                if ($this->maxSize !== null && $value->size > $this->getSizeLimit()) {
                    return array(
                        ...
                        array(
                            ...
                            'formattedLimit' => ArSql::$app->formatter->asShortSize($this->getSizeLimit()),
                        ),
                    );
                    ...
                    return array(
                        ...
                        array(
                            'file' => $value->name,
                            'limit' => $this->minSize,
                            'formattedLimit' => ArSql::$app->formatter->asShortSize($this->minSize),
                        ),
                    );
                } elseif (!empty($this->extensions) && !$this->validateExtension($value)) {
                    return array($this->wrongExtension, array('file' => $value->name, 'extensions' => implode(', ', $this->extensions)));
                } elseif (!empty($this->mimeTypes) &&  !$this->validateMimeType($value)) {
                    return array($this->wrongMimeType, array('file' => $value->name, 'mimeTypes' => implode(', ', $this->mimeTypes)));
                }
                return null;
            case UPLOAD_ERR_INI_SIZE:
            case UPLOAD_ERR_FORM_SIZE:
                return array($this->tooBig, array(
                    'file' => $value->name,
                    'limit' => $this->getSizeLimit(),
                    'formattedLimit' => ArSql::$app->formatter->asShortSize($this->getSizeLimit()),
                ));
            case UPLOAD_ERR_PARTIAL:
                ArSql::warning('File was only partially uploaded: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_NO_TMP_DIR:
                ArSql::warning('Missing the temporary folder to store the uploaded file: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_CANT_WRITE:
                ArSql::warning('Failed to write the uploaded file to disk: ' . $value->name, __METHOD__);
                break;
            case UPLOAD_ERR_EXTENSION:
                ArSql::warning('File upload was stopped by some PHP extension: ' . $value->name, __METHOD__);
                break;
            default:
                break;
        }
    ...
    public function getSizeLimit()
    {
        ...
        if ($postLimit > 0 && $postLimit < $limit) {
            ArSql::warning('PHP.ini\'s \'post_max_size\' is less than \'upload_max_filesize\'.', __METHOD__);
            $limit = $postLimit;
        }
    ...
    public function getClientOptions($model, $attribute)
    {
        ...
        $options = array();
        if ($this->message !== null) {
            $options['message'] = $this->formatMessage($this->message, array(
                'attribute' => $label,
            ));
        }
        ...
        if (!$this->skipOnEmpty) {
            $options['uploadRequired'] = $this->formatMessage($this->uploadRequired, array(
                'attribute' => $label,
            ));
        }
        ...
        if ($this->mimeTypes !== null) {
            ...
            $options['wrongMimeType'] = $this->formatMessage($this->wrongMimeType, array(
                'attribute' => $label,
        ...
        if ($this->extensions !== null) {
            ...
            $options['wrongExtension'] = $this->formatMessage($this->wrongExtension, array(
                'attribute' => $label,
        ...
        if ($this->minSize !== null) {
            ...
            $options['tooSmall'] = $this->formatMessage($this->tooSmall, array(
                'attribute' => $label,
                'limit' => $this->minSize,
                'formattedLimit' => ArSql::$app->formatter->asShortSize($this->minSize),
            ));
        ...
        if ($this->maxSize !== null) {
            ...
            $options['tooBig'] = $this->formatMessage($this->tooBig, array(
                'attribute' => $label,
                'limit' => $this->getSizeLimit(),
                'formattedLimit' => ArSql::$app->formatter->asShortSize($this->getSizeLimit()),
            ));
        ...
        if ($this->maxFiles !== null) {
            ...
            $options['tooMany'] = $this->formatMessage($this->tooMany, array(
                'attribute' => $label,
                'limit' => $this->maxFiles,
            ));
        }
